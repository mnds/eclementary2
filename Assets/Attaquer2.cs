...
public class Attaquer2: MonoBehaviour {
	...
	public bool infligerDegats; //true si on peut infliger des degats
	...
	void Start() {
		...
		//Obtenus par essais successifs
		positionInitiale = new Vector3 (0.42f, -0.09f, 0.6f);
		rotationInitiale = new Quaternion (0f, 0f, 0.6f,0.8f);
		positionFinale = new Vector3 (-0.8f, -0.61f, 0.93f);
		rotationFinale = new Quaternion (0.2f, -0.9f, 0.1f,0.4f);
	}
	
	// Update is called once per frame
	void Update () {
		//Debug.Log ("Angles rotation : " + transform.localRotation);
		if(bypass) return;
	...
	void OnTriggerEnter (Collider objet) {
		...
		if (enTrainDAttaquer && !enCoursDeRetour && infligerDegats) {
			...
			Attaquer2[] attaquers = objetAvecVie.GetComponentsInChildren<Attaquer2>();
			foreach(Attaquer2 a in attaquers)
			{
